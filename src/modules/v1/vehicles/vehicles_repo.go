...
func (vehicles *vehicles_repo) Find(id int) (*models.Vehicles, error) {
	var data models.Vehicles

	result := vehicles.db.Find(&data).RowsAffected
...
func (vehicles *vehicles_repo) Delete(id int) (*gorm.DB, error) {
	var getid models.Vehicles
	if err := vehicles.db.Where("id = ?", id).First(&getid).Error; err != nil {
		return nil, err
	}
...
func (vehicles *vehicles_repo) Update(id int, data *models.Vehicles) (*models.Vehicles, error) {
	var data_from_models models.Vehicles
	result := vehicles.db.Where("id = ?", id).First(&data_from_models).Updates(data)
